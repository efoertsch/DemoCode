...
/**
 * A 'ScanRecord' for Ble advertising data for pre Android 5.0 devices
 * Code cribbed from ScanRecord and BluetoothUuid modified accordingly.
 */
@TargetApi(19)
public class BleScanRecord {
    ...
    public String toString() {
        return "BleScanRecord [mAdvertiseFlags=" + mAdvertiseFlags + ", mServiceUuids=" + mServiceUuids
                + ", mManufacturerSpecificData=" + uuidsToString(mManufacturerSpecificData)
                + ", mServiceData=" + mapToString(mServiceData)
    ...
    private String uuidsToString(SparseArray<byte[]> array) {
        if (array == null) {
            return "null";
        }
        if (array.size() == 0) {
    ...
    private <T> String mapToString(Map<T, byte[]> map) {
        if (map == null) {
            return "null";
        }
        if (map.isEmpty()) {
